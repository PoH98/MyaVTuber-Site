<template>
  <v-sheet class="meme">
    <v-carousel hide-delimiters interval="3000" continuous touchless cycle :show-arrows="false" height="100vh"
      class="background-img">
      <v-carousel-item cover alt="@Handmaker" src="/img/asshurt.jpg" />
      <v-carousel-item cover alt="@Handmaker" src="https://pbs.twimg.com/media/FNxryzMVUAAJudN?format=jpg&name=large" />
      <v-carousel-item cover alt="@Handmaker" src="https://pbs.twimg.com/media/FPGIyCgakAAEK3v?format=jpg&name=large" />
    </v-carousel>
    <v-container class="half-trans" v-if="!content.error">
      <h1 class="text-left my-4">米亞梗字典</h1>
      <v-row v-if="content.length > 0">
        <v-col cols="12" sm="6" md="4" lg="3" v-for="(data, i) in content" :key="'wiki_' + i">
          <v-card class="h-100 meme-card" :to="'/meme/' + data.id" flat>
            <div v-if="data.flatData.cardimg" class="px-4 pt-4">
              <v-img height="200" :src="data.flatData.cardimg" cover />
            </div>
